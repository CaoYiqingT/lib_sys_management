...
<body>
	<div class="x-body">
		...
		<button class="layui-btn"
			onclick="x_admin_show('编辑','<%=request.getContextPath()%>/textBookManage/jcAdd')"
			lay-submit="" lay-filter="sreach">
			<i class="layui-icon"></i>添加教材
		</button>
			...
			<tbody class="x-cate">
				<c:forEach items="${bookList }" var="list">
					<tr cate-id='1' fid='0'>
						...
						<!-- ${list.auditing } -->
						<c:if test="${list.auditing == true}">
							<td><input type="checkbox" name="switch" lay-text="通过|未过"
								checked="checked" lay-skin="switch"
								onclick="member_change(${list.ISBN})"></td>
						</c:if>
						<c:if test="${list.auditing == false}">
							<td><input type="checkbox" name="switch" lay-text="通过|未过"
								lay-skin="switch" onclick="member_change(${list.ISBN})"></td>
						</c:if>

						<td class="td-manage">
							<button class="layui-btn layui-btn layui-btn-xs"
								onclick="member_edit('${list.ISBN }','编辑','<%=request.getContextPath()%>/textBookManage/jcEdit')">
								<i class="layui-icon">&#xe642;</i>编辑
							</button>

							<button class="layui-btn-danger layui-btn layui-btn-xs"
								onclick="member_del(this,'要删除的id',${list.ISBN })"
								href="javascript:;">
